function [Xlift, Fval, manopt_info, Manopt_opts] = update_problem_for_dim_and_solve(problem, lifted_dim, init_point, Manopt_opts)
    ...
    function lifted_init = lift_init_point(X, lifted_manifold, add_noise)
        ...
        if add_noise
            lifted_init = lifted_init + 1e-1 * rand(size(lifted_init));
        else
