function num_ranges = get_num_ranges_from_fpath_names(results_fpaths_pairs)
    [num_pairs, ~] = size(results_fpaths_pairs);
    num_ranges = zeros(num_pairs, 1);
    for i = 1:num_pairs
        ...
        char_name = char(name);
        num_ranges(i) = str2double(char_name(start_idx:end_idx));
    end
end