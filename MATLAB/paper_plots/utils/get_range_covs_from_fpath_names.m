function range_covs = get_range_covs_from_fpath_names(results_fpaths_pairs)
    [num_pairs, ~] = size(results_fpaths_pairs);
    range_stddevs = zeros(num_pairs, 1);
    for i = 1:num_pairs
        ...
        char_name = char(name);
        range_stddevs(i) = str2double(char_name(start_idx:end_idx));
    end
    range_covs = range_stddevs.^2;
end